...
***** Emacs
To use Emacs graphically, enable the =gui= USE flag. And enable the =xft= USE flag to render fonts correctly.
#+begin_src sh
...
=doomemacs/doomemacs= has been cloned to =~/.emacs.d=):
...
+ Look up errors/warnings [[file:faq.org::Common Issues][on the FAQ]] and [[https://github.com/doomemacs/doomemacs/issues][Doom's issue tracker]]. It is possible
